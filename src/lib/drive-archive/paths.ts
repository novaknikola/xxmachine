...
// Re-exported so existing `from './paths'` importers can reach it too.
export { sanitizeArchiveLabel } from './label'
...
export function buildArchiveFilename(opts: {
  ...
  /**
   * User-chosen base name. When present it REPLACES the whole machine prefix
   * (day + source type + id hash + model tag): the point of the feature is that
   * the file is recognisable at a glance. Blank or unsanitisable falls through
   * to the legacy name below, byte-identical to before this existed.
   */
  seriesLabel?: string | null
  /** Set when the file lands in its own per-set subfolder (one carousel). */
  seriesFolder?: string | null
}): string {
  // A seriesId plus a position IS a series. Deliberately no longer also
  // requiring seriesTotal > 1: when every variant of a carousel failed,
  // images.length collapsed to 1, this went false, and idShort fell back to
  // sourceId.slice(0,8) — which for bulk_carousel is the job UUID, identical
  // for every item in the job, with no position tag to tell them apart.
  const inSeries = !!opts.seriesId && opts.seriesIndex != null
  ...
  // Display-only disambiguation. Dedup runs on drive_exports.url_hash, which
  // stores the FULL sha256 and is untouched by this — so four chars is enough.
  // Dropping it entirely would let two runs sharing a label produce two
  // identical names in one folder (Drive permits that) with no way to tell
  // which is which.
  const hash4 = hashUrl(opts.url).slice(0, 4)

  // Inside a per-set subfolder the label is already the folder name; repeating
  // it on every file just makes the listing unreadable.
  if (sanitizeArchiveLabel(opts.seriesFolder)) return `${pos}_${hash4}.${ext}`

  const label = sanitizeArchiveLabel(opts.seriesLabel)
  if (label) return `${label}_${pos}_${hash4}.${ext}`

  // ── legacy name — everything below is unchanged ──────────────────────
  ...
  const positionTag = inSeries || opts.total > 1 ? `_${pos}` : ''
  // `model` moves AFTER idShort+position (not before): a carousel's base
  // slide and its variants are generated by DIFFERENT pipelines (e.g.
  // text2img base + seedream_edit variants), so if model led the filename,
  // Drive would alphabetize all "seedream_edit_*" slides into one cluster
  // and all "text2img_*" bases into a separate one -- tearing every
  // carousel's base away from its own variants. Grouping by idShort first
  // keeps model as a trailing descriptive tag that can't break the order.
