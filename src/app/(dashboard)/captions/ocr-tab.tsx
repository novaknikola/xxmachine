...
import { Upload, X, Loader2, ListTodo, Film, ScanText } from 'lucide-react'
...
export function OcrTab() {
  ...
  async function submit() {
    ...
    setSubmitting(true)
    try {
      toast.loading(`Uploading ${files.length} video${files.length > 1 ? 's' : ''}…`, { id: 'cap-ocr-upload' })
      ...
      const res = await fetch('/api/queue/submit', {
        ...
        body: JSON.stringify({ job_type: 'video_ocr', input: { items } }),
      })
      if (!res.ok) {
      ...
      toast.success(`${items.length} video${items.length > 1 ? 's' : ''} sent to queue`, {
        description: 'On-screen text will be ready to download as a CSV from the Queue tab',
    ...
    <div className="max-w-lg mx-auto space-y-4">
      ...
      <Button className="w-full" onClick={submit} disabled={submitting || files.length === 0}>
        {submitting
          ...
          : <><ListTodo className="w-4 h-4 mr-2" />Extract On-Screen Text ({files.length})</>}
      </Button>
      <p className="text-[10px] text-muted-foreground text-center flex items-center justify-center gap-1">
        <ScanText className="w-3 h-3 shrink-0" />
        Reads whatever text is burned into the first second of frame — captions, memes, on-screen graphics.
      </p>
    </div>
  )
}
